import { Module, Inject, Global, DynamicModule, Provider, OnApplicationShutdown } from '@nestjs/common';
import { ModuleRef } from '@nestjs/core';
import { MongoClient } from 'mongodb';
import { MONGO_CONNECTION_NAME, DEFAULT_MONGO_CONNECTION_NAME, MONGO_MODULE_OPTIONS } from './mongo.constants';
import { MongoModuleAsyncOptions, MongoOptionsFactory, MongoModuleOptions } from './interfaces/options.interface';
import { getClientToken, getDbToken } from './mongo.utils';
...
export class MongoCoreModule implements OnApplicationShutdown {
  ...
  static forRootAsync(options: MongoModuleAsyncOptions): DynamicModule {
    ...
    const dbProvider = {
      ...
      useFactory: (mongoModuleOptions: MongoModuleOptions, client: MongoClient) => client.db(mongoModuleOptions.dbName),
  ...
  private static createAsyncOptionsProvider(options: MongoModuleAsyncOptions): Provider {
    if (options.useFactory) {
      ...
      return {
        provide: MONGO_MODULE_OPTIONS,
        useFactory: async (optionsFactory: MongoOptionsFactory) => await optionsFactory.createMongoOptions(),
